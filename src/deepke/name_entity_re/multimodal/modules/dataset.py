...
class MMPNERProcessor(object):
    def __init__(self, data_path, args) -> None:
        ...
        self.re_dict = ['PER(person)','LOC(location)','ORG(organization)','MISC(other)']
        self.re_dict = ['PER','LOC','ORG','MISC']

    def load_from_file2(self, mode="train"):
        ...
        with open(load_file, "r", encoding="utf-8") as f:
            ...
            for line in lines:
                if line.startswith("IMGID:"):
                    if random.random() > self.args.rate and mode == "train":
                        v = 0
                        continue
                    else:
                        v = 1
    ...
    def initPrompt2(self):
        promptSentenceA = self.cfg.pmts
        # promptSentenceA = "What is the type of word among "
        for name in self.re_dict:
            if 'MISC' in name:
                promptSentenceA = promptSentenceA + 'or ' + name + '. '
            else:
                promptSentenceA = promptSentenceA + name + ', '
        sentence = promptSentenceA
        dict = self.tokenizer.encode_plus(text=sentence, max_length=self.max_seq, truncation=True, padding='max_length')
        input_ids, token_type_ids, attention_mask = dict['input_ids'], dict['token_type_ids'], dict['attention_mask']
        input_ids, token_type_ids, attention_mask = torch.tensor(input_ids).reshape(1, -1), torch.tensor(
            token_type_ids).reshape(1, -1), torch.tensor(attention_mask).reshape(1, -1)
        # prompt_ids.append(input_ids)
        prompt_ids = input_ids
        prompt_type_ids = token_type_ids
        prompt_attention_mask = attention_mask

        return torch.tensor(prompt_ids,dtype=torch.int64).to('cuda'), torch.tensor(prompt_type_ids,dtype=torch.int64).to('cuda'), torch.tensor(prompt_attention_mask,dtype=torch.int64).to('cuda')
        # print(prompt_ids)
    ...
    def initPrompt(self):
        ...
        for name in self.re_dict:
            if 'MISC' in name:
                ...
                promptSentenceA = promptSentenceA + name + ', '
        # promptSentenceA = promptSentenceA + promptSentenceB + promptSentenceA
        sentence = promptSentenceA
        dict = self.tokenizer.encode_plus(text=sentence, max_length=self.max_seq, truncation=True, padding='max_length')
        input_ids, token_type_ids, attention_mask = dict['input_ids'], dict['token_type_ids'], dict['attention_mask']
        input_ids, token_type_ids, attention_mask = torch.tensor(input_ids).reshape(1, -1), torch.tensor(
...
class MMPNERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, word in enumerate(word_list):
            ...
            for m in range(len(token)):
                if m == 0:
                    ...
                    labels_fx.append(self.label_mapping[label])
                else:
                    labels.append(self.label_mapping[label])
                    labels_fx.append(self.label_mapping["X"])
        if len(tokens) >= self.max_seq - 1:
            tokens = tokens[0:(self.max_seq - 2)]
            labels = labels[0:(self.max_seq - 2)]
            labels_fx = labels_fx[0:(self.max_seq - 2)]
        ...
        labels_fx = [self.label_mapping["[CLS]"]] + labels_fx + [self.label_mapping["[SEP]"]] + [self.ignore_idx] * (
                    self.max_seq - len(labels_fx) - 2)
        ...
        if self.img_path is not None:
            ...
            if self.aux_img_path is not None:
                ...
                if self.rcnn_img_path is not None:
                    ...
                    return torch.tensor(input_ids), torch.tensor(token_type_ids), torch.tensor(attention_mask), torch.tensor(labels), torch.tensor(labels_fx), image, aux_imgs, rcnn_imgs

                return torch.tensor(input_ids), torch.tensor(token_type_ids), torch.tensor(attention_mask), torch.tensor(labels), torch.tensor(labels_fx), image, aux_imgs

        assert len(input_ids) == len(token_type_ids) == len(attention_mask) == len(labels)
        return torch.tensor(input_ids), torch.tensor(token_type_ids), torch.tensor(attention_mask), torch.tensor(labels), torch.tensor(labels_fx)