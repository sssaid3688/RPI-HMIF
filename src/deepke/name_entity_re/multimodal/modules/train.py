...
class Trainer(object):
    def __init__(self, train_data=None, dev_data=None, test_data=None, model=None, process=None, label_map=None, args=None, logger=None,  writer=None, prompt_label_token=None) -> None:
        ...
        self.optimizer = None
        self.prompt_label_token = prompt_label_token
        self.step = 0
        self.args = args
    ...
    def train(self):
        ...
        self.logger.info("pmt_lr: {}, label_lr: {}, label_weight_decay: {}, infer_lr: {}, infer_weight_decay:{}".format(
            self.args.pmt_lr,
            self.args.label_lr, self.args.label_weight_decay, self.args.infer_lr, self.args.infer_weight_decay))
        ...
        with tqdm(total=self.train_num_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True, initial=self.step) as pbar:
            ...
            for epoch in range(1, self.args.num_epochs+1):
                ...
                for batch in self.train_data:
                    ...
                    attention_mask, labels, labels_fx, logits, loss = self._step(batch, self.prompt_label_token, mode="train")
            ...
            self.logger.info("pmt_lr: {}, label_lr: {}, label_weight_decay: {}, infer_lr: {}, infer_weight_decay:{}".format(self.args.pmt_lr,
                                self.args.label_lr, self.args.label_weight_decay, self.args.infer_lr, self.args.infer_weight_decay))
            self.logger.info(
                "pmtlen:{}, routnum:{}, rate:{}".format(
                    self.args.pmtlen, self.args.routnum, self.args.rate))
            self.logger.info(
                "discrete prompts: {}".format(
                    self.args.pmts))
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                for batch in self.dev_data:
                    ...
                    attention_mask, labels, labels_fx, logits, loss = self._step(batch, self.prompt_label_token, mode="dev")    # logits: batch, seq, num_labels
                    total_loss += loss.detach().cpu().item()

                    if isinstance(logits, torch.Tensor):    
                        logits = logits.argmax(-1).detach().cpu().numpy()  # batch, seq, 1
                    label_ids = labels_fx.detach().cpu().numpy()
                ...
                if f1_score >= self.best_dev_metric:  # this epoch get best performance
                    ...
                    if self.args.save_path is not None: # save model
                        ...
                        torch.save(self.model, self.args.save_path + "/best_model.pt")
    ...
    def _step(self, batch, prompt_label_token, mode="train"):
        input_ids, token_type_ids, attention_mask, labels, labels_fx, images, aux_imgs, rcnn_imgs = batch
        logits, loss = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, labels=labels, images=images, aux_imgs=aux_imgs, rcnn_imgs=rcnn_imgs, prompt_label_token=prompt_label_token)
        return attention_mask, labels, labels_fx, logits, loss
    ...
    def multiModal_before_train(self):
        ...
        for name, param in self.model.named_parameters():
            if 'text' in name and 'pmt' not in name:
                params['params'].append(param)
        parameters.append(params)

         # vit lr
        params = {'lr':1e-5, 'weight_decay':1e-2}
        params['params'] = []
        for name, param in self.model.named_parameters():
            if 'vision' in name:
                params['params'].append(param)
        ...
        params = {'lr': self.args.pmt_lr, 'weight_decay': 1e-2}
        params['params'] = []
        for name, param in self.model.named_parameters():
            if 'pmt' in name:
                params['params'].append(param)
        ...
        params = {'lr': self.args.label_lr, 'weight_decay': self.args.label_weight_decay}
        params['params'] = []
        for name, param in self.model.named_parameters():
            if 'label' in name:
                params['params'].append(param)
        ...
        params = {'lr': self.args.infer_lr, 'weight_decay': self.args.infer_weight_decay}
        params['params'] = []
        for name, param in self.model.named_parameters():
            if 'rou' in name:
                params['params'].append(param)
