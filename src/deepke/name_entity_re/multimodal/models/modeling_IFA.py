...
class dyRouting(nn.Module):
    def __init__(self, config):
        ...
        self.ffn5 = FFN_noz(768,256,768)
        self.ffn6 = FFN_noz(768,256,768)
...
class PromptBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.neftune = NEFTune(noise_alpha=5)
        # self.promptEmbedding = [nn.ModuleList(nn.Embedding(config.type_vocab_size, config.hidden_size) for _ in range(10))]
        ...
        self.pmtlen=config.pmtlen
        self.mlp=MLP(config)

    def forward(
        self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None,
            past_key_values_length=0,
            prompt_extended_attention_mask=None,
            batch_size=None
    ):
        ...
        token_type_ids = torch.repeat_interleave(token_type_ids, repeats=batch_size, dim=0)
        position_ids = torch.repeat_interleave(position_ids, repeats=batch_size, dim=0)
        ...
        prompt_embeddings = torch.randn([batch_size,self.pmtlen,self.config.hidden_size]).to(inputs_embeds.device)
        prompt_embeddings = self.mlp(prompt_embeddings)
        inputs_embeds = torch.cat([prompt_embeddings, inputs_embeds], dim=1)[:,:40,:]
        ...
        prompt_extended_attention_mask[:,prompt_extended_attention_mask.sum():prompt_extended_attention_mask.sum()+self.pmtlen]=1
        prompt_extended_attention_mask=torch.repeat_interleave(prompt_extended_attention_mask, repeats=batch_size, dim=0)
        return embeddings, prompt_extended_attention_mask
...
#         x_s=x
#         y_s=y
#         w_x, w_y, x_s, y_s = self.updRouterWt(x_s, y_s, t)
#         # x = w_x * x
#         # y = w_y * y
#         # return x+y
#         z = self.atten(x_s, y_s, mask)[0]
#         return z
#
#     def updRouterWt(self, x, y, t): #t：迭代次数
#         #x, y [bsz, hidden_size]
...
#                 b1 = b1+torch.diagonal(torch.matmul(a,x.transpose(-1,-2)),dim1=-2,dim2=-1)
#                 b2 = b2+torch.diagonal(torch.matmul(a,y.transpose(-1,-2)),dim1=-2,dim2=-1)
#         return c1, c2, x_s, y_s

class dyRouting5(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.ffn1 = FFN_noz(768,512,768)
        self.ffn2 = FFN_noz(768,512,768)
        # self.ffn3 = FFN_noz(40,40,1)
        # self.ffn4 = FFN_noz(40,40,1)
        self.squaAct = squashing()
        self.cs = config.routnum
        # self.routing_weight = self.updRouterWt()
        self.atten = CoAttentionBertLayer_txt(config)
    ...
    def updRouterWt(self, x, y, t): #t：迭代次数
        ...
        for i in range(0,self.cs):
            if i == 0:
                ...
                # bsz = int(cat.shape[0]/2)
                # c1,c2=cat[:bsz],cat[bsz:]
            x_s = self.ffn1(x_s) + c1 * x_s
            y_s = self.ffn2(y_s) + c2 * y_s
            # print(self.mode)
            # print("c1:",c1.shape,"x:",x.shape,"c2:",c2.shape,"y:",y.shape)
            a = self.squaAct(x_s+y_s)
            if i == 0:
                b1 = torch.diagonal(torch.matmul(a,x.transpose(-1,-2)),dim1=-2,dim2=-1)
                b2 = torch.diagonal(torch.matmul(a,y.transpose(-1,-2)),dim1=-2,dim2=-1)
            else:
...
class dyRouting4(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        # self.ffn1 = FFN_noz(self.hidden_size,512,self.hidden_size)
        ...
        self.squaAct = squashing()
        self.cs = config.routnum
        # self.routing_weight = self.upd RouterWt()
        self.atten = CoAttentionBertLayer_txt(config)
    ...
    def updRouterWt(self, x, y, t): #t：迭代次数
        ...
        for i in range(0,self.cs):
            ...
            a = self.squaAct(x_s+y_s)
            if i == 0:
                b1 = torch.diagonal(torch.matmul(a,x_s.transpose(-1,-2)),dim1=-2,dim2=-1)
                b2 = torch.diagonal(torch.matmul(a,y_s.transpose(-1,-2)),dim1=-2,dim2=-1)
            else:
                b1 = b1+torch.diagonal(torch.matmul(a,x_s.transpose(-1,-2)),dim1=-2,dim2=-1)
                b2 = b2+torch.diagonal(torch.matmul(a,y_s.transpose(-1,-2)),dim1=-2,dim2=-1)
        return c1, c2, x_s, y_s


class dyRouting3(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        # self.ffn1 = FFN_noz(768,256,1)
...
class BottleneckFusion(nn.Module):
    def __init__(self, vision_config, text_config, bottlenecknum=4, layernum=4):
        super().__init__()
        self.vision_config = vision_config
        self.text_config = text_config
        self.layernum=layernum
        self.parainit = nn.Parameter
        self.bottlenecknum = bottlenecknum
        self.vision_layers = nn.ModuleList([CLIPEncoderLayer(vision_config) for _ in range(layernum)])
        self.text_layer = nn.ModuleList([BertLayer(text_config) for _ in range(layernum)])
    ...
    def forward(
        ...
        self.bottleneck_tokens = self.parainit(torch.randn(vision_embeds.shape[0],self.bottlenecknum, self.text_config.hidden_size)).to(text_embeds.device)
        attention_mask = torch.cat([attention_mask,torch.ones([attention_mask.shape[0],1,1,self.bottlenecknum]).to(text_embeds.device)],dim=-1)
        for idx in range(self.layernum):
            ...
            output_qks = True
            vision_hidden_states = torch.cat([vision_hidden_states, self.bottleneck_tokens],dim=-2)
            ...
            vision_hidden_states = vision_layer_output[0][:,:-self.bottlenecknum,:]
            self.bottleneck_tokens = vision_layer_output[0][:,-self.bottlenecknum:,:]
            # text
            # TODO: 9-12 layers past vison qks to text

            layer_head_mask = head_mask[idx] if head_mask is not None else None
            text_hidden_states = torch.cat([text_hidden_states,self.bottleneck_tokens],dim=-2)
            ...
            text_hidden_states = text_layer_output[0][:,:-self.bottlenecknum,:]
            self.bottleneck_tokens = text_layer_output[0][:,-self.bottlenecknum:,:]
            if output_attentions:
...
class label_guide_encoder_noimg(nn.Module):
    ...
    def forward(self,
        ...
        return text,
class label_guide_encoder_nolabel(nn.Module):
    def __init__(self, text_config, image_config):
        super().__init__()
        self.text_config = text_config
        self.image_config = image_config
...
class label_guide_encoder(nn.Module):
    def __init__(self, text_config, image_config):
        ...
        self.image_config = image_config
        self.label_guide_attention_txt = CoAttentionBertLayer_txt(self.text_config)
        # self.label_guide_attention_txt2 = CoAttentionBertLayer_txt(self.text_config)
        self.label_guide_attention_image_txt = CoAttentionLayer_image(self.text_config)
        self.label_guide_attention_image = CoAttentionLayer_image(self.image_config)
        self.ffn = FFN(40,40,768)
        self.ffn2 = FFN(61, 40, 768)
        # self.ffn_1 = FFN_noz(768,512,768)
        # self.ffn_2 = FFN_noz(768,512,768)
    ...
    def forward(self,
        ...
        text_out2 = self.label_guide_attention_txt(label_embeds,text_embeds,attention_mask_label)[0]
        text = text_out+self.ffn(text_out2)
        img_out = self.label_guide_attention_image_txt(text,image_embeds,attention_mask_txt, past_key_values = (image_embeds,))[0]
        img_out2 = self.label_guide_attention_image(image_embeds,text)[0]
        img=img_out+self.ffn2(img_out2)
        return text,img
...
class IFAModel(nn.Module):
    def __init__(self, vision_config, text_config, add_pooling_layer=True):
        ...
        self.promt_text_embeddings = PromptBertEmbeddings(text_config)
        ...
        # self.ffns=FFN(61,40,768)
        self.router = dyRouting4(text_config)
        # self.bottleneck = BottleneckFusion(text_config=text_config,vision_config=vision_config)
    ...
    def forward(
        ...
        prompt_embedding_output, prompt_extended_attention_mask = self.promt_text_embeddings(
            input_ids=prompt_label_token[0],
            position_ids=position_ids,
            token_type_ids=prompt_label_token[1],
            prompt_extended_attention_mask=prompt_label_token[2],
            batch_size=batch_size,
        )
        prompt_extended_attention_mask: torch.Tensor = get_extended_attention_mask(prompt_extended_attention_mask,
        ...
        pmtencoder_out = self.pmtenocoder(
            text_embeds=prompt_embedding_output,  # 换成promt描述
